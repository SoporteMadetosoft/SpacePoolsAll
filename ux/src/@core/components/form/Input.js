...
export const Input = ({ name, label, className = '', placeholder = label, containerClassname = 'mt-1', type = "text", styles, errMsg = 'Campo requerido', readOnly = false, disabled = false, symbol = false, defecto = "", position, zone }) => {
        ...
        <div className={containerClassname} >
            ...
            <InputGroup>
                <input
                    ...
                    value={zone ? normalForm[zone][position][name] : normalForm[name] ? normalForm[name] : defecto}
                    onChange={zone ? ({ target }) => dispatch(editRepeaterRegister(zone, position, { name: target.name, value: target.value })) : handleInputChange}
                    autoComplete="off"
                />
                { symbol && (<InputGroupAddon addonType='append'><InputGroupText>{symbol}</InputGroupText></InputGroupAddon>) }
            </InputGroup>
        </div>
    )
}